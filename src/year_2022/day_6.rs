#[aoc_generator(day6)]
fn generator(input: &[u8]) -> Vec<u32> {
    input.iter().map(|c| 1 << ((*c - b'a') as u32)).collect()
}

fn bithash(slice: &[u32]) -> u32 {
    slice.iter().fold(0, |acc, n| acc ^ n)
}

fn full_hash(input: &[u32], count: usize) -> usize {
    count
        + input
            .windows(count)
            .map(bithash)
...
fn rolling_hash(input: &[u32], count: usize) -> usize {
...
#[aoc(day6, part1)]
fn with_regular_hash_4(input: &[u32]) -> usize {
    full_hash(input, 4)
}
#[aoc(day6, part2)]
fn with_regular_hash_14(input: &[u32]) -> usize {
    full_hash(input, 14)
}
#[aoc(day6, part1, rolling)]
fn with_rolling_hash_4(input: &[u32]) -> usize {
    rolling_hash(input, 4)
}
#[aoc(day6, part2, rolling)]
fn with_rolling_hash_14(input: &[u32]) -> usize {
